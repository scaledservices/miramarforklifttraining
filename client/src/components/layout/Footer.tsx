    ...
    <footer className="bg-brand-dark text-white" data-testid="footer">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 py-12">
          <div>
            <div className="flex items-center gap-2 mb-4">
              <Logo variant="full" theme="dark" loading="lazy" />
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              ...
              <li><Link href="/business" className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-link-business">{t("footer.businessSolutions")}</Link></li>
              <li><Link href="/renewal" className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-link-renewal">{t("footer.renewal")}</Link></li>
              <li><Link href="/request-quote" className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-link-quote">{t("footer.requestQuote")}</Link></li>
              <li><Link href="/forklift-certification-cost" className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-link-cost">{t("footer.certificationCost")}</Link></li>
              <li><Link href="/forklift-certification-near-me" className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-link-near-me">{t("footer.findTrainingNearMe")}</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-2">
                ...
                <a href={`tel:${brand.support.phoneTel}`} className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-phone">{brand.support.phone}</a>
              </li>
              <li className="flex items-center gap-2">
                <Mail className="w-4 h-4 text-accent shrink-0" />
                <a href={`mailto:${brand.support.infoEmail}`} className="text-sm opacity-75 transition-opacity hover:opacity-100" data-testid="footer-email">{brand.support.infoEmail}</a>
              </li>
        ...
        <div className="border-t border-white/15 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs opacity-60 transition-opacity hover:opacity-100">
            &copy; {new Date().getFullYear()} {brand.name}. {t("footer.rights")}
          </p>
          <div className="flex items-center gap-4 flex-wrap justify-center">
            <Link href="/terms" className="text-xs opacity-60 transition-opacity hover:opacity-100" data-testid="footer-link-terms">{t("footer.terms")}</Link>
            <span className="text-xs opacity-30">|</span>
            <Link href="/privacy" className="text-xs opacity-60 transition-opacity hover:opacity-100" data-testid="footer-link-privacy">{t("footer.privacy")}</Link>
            <span className="text-xs opacity-30">|</span>
            <Link href="/refund-policy" className="text-xs opacity-60 transition-opacity hover:opacity-100" data-testid="footer-link-refund">{t("footer.refundPolicy")}</Link>
            <span className="text-xs opacity-30">|</span>
            <Link href="/support" className="text-xs opacity-60 transition-opacity hover:opacity-100" data-testid="footer-contact-link">{t("footer.contactUs")}</Link>
          </div>
        </div>
      </div>
    </footer>
